...
def main():
    ...
    # Dataset data may already be materialized by the orchestration layer.
    # Keep the original ClearML association when a dataset name is supplied;
    # persistent-path runs can leave it empty and skip this metadata-only lookup.
    if args.clearml_dataset_name:
        ClearMLDataset.get(
